...
export const MyAnalyze = () => {
  const { user, analyzes } = AuthData();
  ...
  const fetchHistory = async () => {
    try {
      const response = await axios.get(`http://127.0.0.1:8000/analyze/get_patsient_analyzes?id=${user.id}`);
  ...
  return (
    <div className={styles.recordhistory}>
      <div className={styles.titleDiv}>
        <h2 className={styles.titlerecord}>Мои анализы</h2>
        <img 
